...
def buat_division_summary_rows(df):
    ...
    if 'Source Division' not in df.columns:
        df_i = df[~is_competitor(df.get('Produsen', df['Product Code']))]
        df_k = df[ is_competitor(df.get('Produsen', df['Product Code']))]
        fi = df_i['Facing'].sum()
        fk = df_k['Facing'].sum()
        total = fi + fk
...
def buat_category_divisi_section(df, periods, targets):
    ...
    """
    is_comp = is_competitor(df.get('Produsen', df['Product Code']))
